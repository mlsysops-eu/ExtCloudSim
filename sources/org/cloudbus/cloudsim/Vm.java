...
public class Vm {
	...
	private int wasInMigration;
	
	private double request_previous;

	private double price;
	
	private double migpriority;
	...
	public Vm(
		...
		setPrice(price);
		setWasInMigration(0);
		set_vm_normalized_alloc(0);
		set_vm_normalized_req(0);
		//set_cpu_normalized_alloc(0);
		//set_cpu_normalized_req(0);
		set_vm_normalized_req_previous(1.0);
	...
	public List<Double> getCurrentRequestedMips() {
		...
		List<Double> currentRequestedMips = getCloudletScheduler().getCurrentRequestedMips(); //Cloudlets influence VM utilization
	...
	// BS defined
	public List<Double> getCurrentRequestedMipsMax() {
		List<Double> currentRequestedMipsMax = getCloudletScheduler().getCurrentRequestedMipsMax(); //Cloudlets influence VM utilization - max
		if (isBeingInstantiated()) {
			currentRequestedMipsMax = new ArrayList<Double>();
			for (int i = 0; i < getNumberOfPes(); i++) {
				currentRequestedMipsMax.add(getMips());
			}
		}
		return currentRequestedMipsMax;
	}
	
	// BS defined
	public double getCurrentRequestedTotalMipsMax() {
		double totalRequestedMipsMax = 0;
		for (double mips : getCurrentRequestedMipsMax()) {
			totalRequestedMipsMax += mips;
		}
		return totalRequestedMipsMax;
	}
	...
	public double get_vm_normalized_alloc () {
		return allocated;
	}
	public void set_vm_normalized_alloc (double allocated) {
		this.allocated = allocated;
	}
	
	public double get_vm_normalized_req () {
		return requested;
	}
	public void set_vm_normalized_req (double requested) {
		this.requested = requested;
	}
	
	public double get_vm_normalized_req_previous () {
		return request_previous;
	}
	public void set_vm_normalized_req_previous (double requested_previous) {
		this.request_previous = requested_previous;
	}
	
	public void set_Class (int vmClass) {
	...
	public void setWasInMigration( int wasInMigration) {
		this.wasInMigration = wasInMigration;
	}
	
	public int getWasInMigration( ) {
		return wasInMigration;
	}
	...
	public void setHistory (double predictedUtilization) {
		this.predictedUtilization = predictedUtilization;
	}
	...
	public int getMigrationSlotNumber (int ram) {
		double time = ram/125;
		int slot = (int)(time/Constants.TIME_SLOT) + 1;
