...
public class CloudletSchedulerDynamicWorkload extends CloudletSchedulerTimeShared {
	...
	@Override
	public double getTotalUtilizationOfCpuMax(double time) {
		double totalUtilization = 0;
		for (ResCloudlet rcl : getCloudletExecList()) {
			totalUtilization += rcl.getCloudlet().getUtilizationOfCpuMax(time);
		}
		return totalUtilization;
	}
	...
	public List<Double> getCurrentRequestedMipsMax() {
		...
		List<Double> currentMips = new ArrayList<Double>();
		double totalMips = getTotalUtilizationOfCpuMax(getPreviousTime()) * getTotalMips();
		double mipsForPe = totalMips / getNumberOfPes();
